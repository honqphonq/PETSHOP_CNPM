...
const createVnpPayUrl = require('../helpers/createVnpPayUrl')
...
class IndexController {
  ...
  getCheckoutPage(req, res, next) {
    if (req.isAuthenticated()) {
      ...
      customers.findOne({ 'loginInformation.userName': user }, (err, customerResult) => {
        ...
        const totalProductPrice = customerResult.listProduct.reduce((acc,{productPrice, amount})=> acc + productPrice * Number(amount), 0)
        res.render("checkout", { customer: customerResult, billPrice: totalProductPrice + deliveryFee });
      });
    } else {
      res.redirect('/login');
    }
 ...
 async postCheckout(req, res, next) {
    if (req.isAuthenticated()) {
      ...
      const { ward, city, address, district } = req.query;

    const response = await region.find({})
      console.log('response',response)
      customers.findOne({ 'loginInformation.userName': user }, (err, customerResult) => {
        region.findOne({Id: String(city)}, (err, cityResult) => {
          console.log('err',err)
          ...
          newBill.save(data)
            .then(() => {
              ...
              customers.findOneAndUpdate({_id: customerResult._id}, {
                listProduct: []
              }, { new: true })
              .then(() =>{
                res.redirect('/');
  ...
  createPayment (req, res) {
    const {ward, city, district, address} = req.body
    const { billPrice } = req.query
    const ipAddr = req.headers['x-forwarded-for'] ||
        req.connection.remoteAddress ||
        req.socket.remoteAddress ||
        req.connection.socket.remoteAddress;
    const redirectUrl = createVnpPayUrl({
      ward,
      city,
      district,
      address,
